...
class StockCubit extends Cubit<StockState> {
  ...
  void addOrUpdateDetailedStock(int index, StockDetailModel model) {
    ...
    final sameStockIndex = updatedProducts[index]
        .stockDetailModel
        .indexWhere((element) => element.title?.toLowerCase() == model.title?.toLowerCase());
    // final isStockOld = updatedProducts[index].stockDetailModel?.where((element) => element.title == model.title);
    if (sameStockIndex != -1) {
      updatedProducts[index].stockDetailModel[sameStockIndex].meter =
          (updatedProducts[index].stockDetailModel[sameStockIndex].meter ?? 0) + (model.meter ?? 0);
    } else {
  ...
  Future<void> addProduct(StockModel model, {int? id}) async {
    final indexOfSameProduct =
        lists?.indexWhere((element) => element.title?.toLowerCase() == model.title?.toLowerCase());
    if (indexOfSameProduct != -1 && indexOfSameProduct != null) {
      lists?[indexOfSameProduct].totalMeter =
          lists?[indexOfSameProduct].totalMeter ?? 0 + (model.totalMeter ?? 0);
